...
async function callGemini(model: string, requestBody: any) {
    ...
    try {
        ...
        console.error(`Error calling Gemini model ${model}:`, error.response?.data || error.message);
...
// This function remains with the SDK as it's more complex with file URIs
export const generateFromDocument = onCall(
    ...
    async (request) => {
        ...
        try {
            ...
            console.error("Error in generateFromDocument with SDK:", e);
            throw new HttpsError("internal", "An error occurred in the vision model function.");
        }
    }
);
