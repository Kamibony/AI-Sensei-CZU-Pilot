# Este script executa o refactoring do projeto AI Sensei para uma forma modular.
...
# Escreve o conteúdo nos arquivos
try {
    ...
    Set-Content -Path (Join-Path $jsDirPath "api.js") -Value $apiJsContent -Encoding UTF8
    Write-Host "✅ Arquivo 'public/js/api.js' foi criado/atualizado com sucesso." -ForegroundColor Green

    Set-Content -Path (Join-Path $jsDirPath "editor-handler.js") -Value $editorHandlerJsContent -Encoding UTF8
    Write-Host "✅ Arquivo 'public/js/editor-handler.js' foi criado/atualizado com sucesso." -ForegroundColor Green

    Set-Content -Path "public/index.html" -Value $indexHtmlContent -Encoding UTF8
    Write-Host "✅ Arquivo 'public/index.html' foi atualizado com sucesso." -ForegroundColor Green
    
    Write-Host "🎉 Refactoring concluído! Você pode iniciar o ambiente de desenvolvimento." -ForegroundColor Cyan
}
catch {
    Write-Host "❌ Ocorreu um erro ao escrever os arquivos:" -ForegroundColor Red
    Write-Host $_
}