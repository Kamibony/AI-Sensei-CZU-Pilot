...
export class ProfessorInteractionsView extends LitElement {
    ...
    _selectConversation(studentId, studentName) {
        ...
        this.messagesUnsubscribe = onSnapshot(messagesQuery, (querySnapshot) => {
            this._messages = querySnapshot.docs.map(doc => ({id: doc.id, ...doc.data()}));
            this._isLoadingMessages = false;
    ...
    // Pomocná funkcia pre farbu avatara (ako u študentov)
     _getAvatarColor(id) {
    ...
    // Handler pre vyhľadávanie konverzácií
    _handleSearchInput(e) {
        this._searchTerm = e.target.value.toLowerCase();
    }

    // Filtrované konverzácie
    get _filteredConversations() {
        if (!this._searchTerm) {
            return this._conversations;
        }
        return this._conversations.filter(conv =>
            (conv.studentName?.toLowerCase() || '').includes(this._searchTerm)
        );
    }

    // --- Renderovacie Metódy ---

    // === ZMENENÁ METÓDA - Renderovanie Karty Konverzácie ===
    renderConversationCard(conv) {
        const isSelected = this._selectedStudentId === conv.studentId;
        const bgClass = isSelected ? 'bg-slate-200 border-l-4 border-green-600' : 'bg-white hover:bg-slate-50 border-l-4 border-transparent';
        ...
        return html`
            <div class="conversation-card border-b border-slate-100 cursor-pointer transition-colors ${bgClass}"
                 @click=${() => this._selectConversation(conv.studentId, conv.studentName)}>
                 <div class="p-4 flex items-center space-x-3">
                    <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center text-white font-medium text-sm ${avatarBgColor}">
                        ${initials}
                    </div>
