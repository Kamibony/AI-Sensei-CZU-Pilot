...
export class StudentDashboardView extends LitElement {
    ...
    async _fetchRecentLessons(groupIds) {
        // Fetch most recent lessons assigned to student's groups
        try {
    ...
    render() {
        ...
        const heroLesson = this._recentLessons.length > 0 ? this._recentLessons[0] : null;
        const nextUpLessons = this._recentLessons.slice(1, 3); // Get next 2
        ...
        return html`
            <div class="space-y-8 pb-24 px-4 md:px-0">
                ...
                <!-- B. "My Classes" (Rectangular) -->
                <div>
                    <div class="flex overflow-x-auto gap-4 pb-4 -mx-4 px-4 no-scrollbar snap-x">

                        ${this._groups.map(group => html`
                            <div class="min-w-[140px] h-24 bg-white border border-slate-200 rounded-xl flex flex-col justify-center items-center shadow-sm hover:border-indigo-400 transition-all cursor-pointer group snap-start" @click=${() => showToast(`Třída: ${group.name}`)}>
                                <div class="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600 text-xs font-black mb-2 border border-indigo-100 group-hover:scale-110 transition-transform">
                                    ${group.name.substring(0, 2).toUpperCase()}
                                </div>
                                <span class="text-xs font-bold text-slate-700 truncate max-w-[90%]">${group.name}</span>
                            </div>
                        `)}

                        <!-- Add/Join Class Action -->
                        <div class="min-w-[140px] h-24 bg-slate-50 border border-dashed border-slate-300 rounded-xl flex flex-col justify-center items-center cursor-pointer hover:bg-slate-100 transition-all snap-start" @click=${() => this._showJoinModal = true}>
                            <span class="text-xl text-slate-400 font-bold mb-1">+</span>
                            <span class="text-xs font-bold text-slate-500">${t('student.join')}</span>
                        </div>
                    </div>
                </div>

                <!-- C. "Jump Back In" (Hero Card - Wide) -->
                <div>
                    ${heroLesson ? html`
                        <div class="w-full p-6 bg-white border-l-4 border-indigo-600 rounded-xl shadow-md flex justify-between items-center cursor-pointer hover:shadow-lg transition-all group"
                             @click=${() => this._handleLessonSelected(heroLesson.id)}>
                            <div class="flex-1 mr-4">
                                <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">${t('student_dashboard.jump_back') || 'POKRAČOVAT V LEKCI'}</p>
                                <h2 class="text-xl font-bold text-slate-900 line-clamp-1 group-hover:text-indigo-600 transition-colors">
                                    ${heroLesson.title}
                                </h2>
                                ${heroLesson.subtitle ? html`<p class="text-sm text-slate-500 line-clamp-1 mt-1">${heroLesson.subtitle}</p>` : ''}
                            </div>

                            <button class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-lg font-bold text-sm transition-colors flex items-center gap-2 shadow-sm whitespace-nowrap">
                                <span>▶️</span>
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        ${nextUpLessons.length > 0 ? nextUpLessons.map(lesson => html`
                            <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex items-center gap-4 hover:border-indigo-300 transition-all cursor-pointer"
                                ...
                                <div class="w-10 h-10 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0 font-bold text-lg">
                                    📚
                                </div>
                                <div class="flex-1">
                                    <h4 class="font-bold text-slate-900 line-clamp-1">${lesson.title}</h4>
                                    <p class="text-xs text-slate-500 font-medium line-clamp-1">${lesson.subtitle || 'Lekce'}</p>
                                </div>
                                <div class="text-right">
                                    <span class="block text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md">Nový</span>
                                </div>
                            </div>
                        `) : html`
