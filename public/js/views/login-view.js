...
export class LoginView extends Localized(LitElement) {
    static properties = {
    ...
    async _handleLogin(e) {
        ...
        try {
            ...
            let message = this.t('auth.error_login_generic');
            switch (error.code) {
                case 'auth/invalid-credential':
                case 'auth/user-not-found':
                case 'auth/wrong-password':
                    message = this.t('auth.error_invalid_credential');
                    break;
                case 'auth/invalid-email':
                    message = this.t('auth.error_invalid_email');
                    break;
                case 'auth/too-many-requests':
                    message = this.t('auth.error_too_many_requests');
                    break;
                case 'auth/user-disabled':
                    message = this.t('auth.error_user_disabled');
                    break;
            }
            this._error = message;
        } finally {
    ...
    async _handleRegister(e) {
        ...
        if (!name || name.trim().length < 2) {
            this._error = this.t('auth.error_name_required');
            return;
        }
        ...
        try {
            ...
            showToast(this.t('auth.success_register'), 'success');
        } catch (error) {
            console.error("Error during registration:", error);
            let message = this.t('auth.error_register_generic');
            const errorCode = error.details?.errorCode;
            switch (errorCode) {
                case 'auth/email-already-in-use':
                    message = this.t('auth.error_email_in_use');
                    break;
                case 'auth/weak-password':
                    message = this.t('auth.error_weak_password');
                    break;
                case 'auth/invalid-email':
                    message = this.t('auth.error_invalid_email');
                    break;
        ...
        return html`
            <div class="animate-fade-in w-full max-w-lg">
                <div class="text-center mb-8">
                    <h2 class="text-3xl font-bold text-slate-900">${this.t('auth.role_selection')}</h2>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <button @click=${() => this._selectRole('professor')}
                        ...
                        <div>
                            ...
                            <h3 class="text-lg font-bold text-slate-900 group-hover:text-indigo-600 transition-colors">${this.t('auth.role_professor')}</h3>
                    ...
                    <button @click=${() => this._selectRole('student')}
                        ...
                        <div>
                            ...
                            <h3 class="text-lg font-bold text-slate-900 group-hover:text-green-600 transition-colors">${this.t('auth.role_student')}</h3>
        ...
        return html`
            <div class="w-full max-w-md space-y-8 animate-fade-in">
                <button @click=${this._backToRoles} class="text-sm font-bold text-slate-400 hover:text-slate-700 flex items-center transition-colors mb-4">
                    ...
                    ${this.t('auth.back_to_roles')}
                </button>

                <div class="text-center lg:text-left">
                     <h2 class="text-3xl font-bold text-slate-900">${this._isRegistering ? this.t('auth.create_account') : this.t(titleKey)}</h2>
                     <p class="text-slate-500 mt-2">${this._isRegistering ? '' : this.t('auth.welcome_desc')}</p>
                </div>

                <div class="${this._isRegistering ? 'hidden' : 'block'} animate-fade-in">
                    <form @submit=${this._handleLogin} class="space-y-5">
                        <div class="space-y-1">
                            <label for="login-email" class="text-sm font-medium text-slate-700">${this.t('auth.email_label')}</label>
                            <input type="email" id="login-email" placeholder="${this.t('auth.email_placeholder')}" autocomplete="email" required
                        ...
                        <div class="space-y-1">
                            <label for="login-password" class="text-sm font-medium text-slate-700">${this.t('auth.password_label')}</label>
                            <input type="password" id="login-password" placeholder="${this.t('auth.password_placeholder')}" autocomplete="current-password" required
                                class="${isProfessor ? 'focus:ring-indigo-500' : 'focus:ring-green-500'} w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:border-transparent transition-all">
                        </div>
                        ...
                        <button type="submit" ?disabled=${this._isLoading}
                            ...
                            ${this._isLoading ? this.t('common.loading') : this.t('auth.login_btn')}
                        </button>
                    </form>
                ...
                <div class="${this._isRegistering ? 'block' : 'hidden'} animate-fade-in">
                    <form @submit=${this._handleRegister} class="space-y-5">
                        ...
                        <div class="space-y-1">
                            <label for="register-name" class="text-sm font-medium text-slate-700">${this.t('auth.name_label')}</label>
                            <input type="text" id="register-name" placeholder="${this.t('auth.name_placeholder')}" autocomplete="name" required
                                class="${isProfessor ? 'focus:ring-indigo-500' : 'focus:ring-green-500'} w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:border-transparent transition-all">
                        </div>

                        <div class="space-y-1">
                            <label for="register-email" class="text-sm font-medium text-slate-700">${this.t('auth.email_label')}</label>
                            <input type="email" id="register-email" placeholder="${this.t('auth.email_placeholder')}" autocomplete="email" required
                                class="${isProfessor ? 'focus:ring-indigo-500' : 'focus:ring-green-500'} w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:border-transparent transition-all">
                        </div>
                        <div class="space-y-1">
                            <label for="register-password" class="text-sm font-medium text-slate-700">${this.t('auth.password_label')}</label>
                            <input type="password" id="register-password" placeholder="${this.t('auth.password_placeholder')}" autocomplete="new-password" required
                                class="${isProfessor ? 'focus:ring-indigo-500' : 'focus:ring-green-500'} w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:border-transparent transition-all">
                        </div>
                        ...
                        <button type="submit" ?disabled=${this._isLoading}
                            ...
                            ${this._isLoading ? this.t('common.loading') : this.t('auth.register_btn')}
                        </button>
                    </form>
                        <div class="mt-6 text-center">
                        <p class="text-slate-600">
                            ${this.t('auth.have_account')}
                            <a href="#" @click=${this._toggleMode} class="${isProfessor ? 'text-indigo-700 hover:text-indigo-800' : 'text-green-700 hover:text-green-800'} font-bold hover:underline transition-colors">${this.t('auth.login_link')}</a>
                        </p>
                    </div>
                </div>

                <p class="text-center text-xs text-slate-400 mt-8">
                    ${this.t('auth.footer_rights')}
                </p>
            </div>
        `;
    }
            ...
            <div class="hidden lg:flex lg:w-1/2 relative bg-gradient-to-br from-indigo-900 to-slate-900 overflow-hidden items-center justify-center">
                ...
                <div class="relative z-10 p-12 text-white max-w-xl">
                    ...
                    <h1 class="text-5xl font-extrabold tracking-tight leading-tight mb-6">
                        ${this.t('auth.branding_title_1')} <br> <span class="text-indigo-400">${this.t('auth.branding_title_2')}</span>
                    </h1>
                    <p class="text-lg text-slate-300 leading-relaxed">
                        ${this.t('auth.branding_desc')}
                    </p>
