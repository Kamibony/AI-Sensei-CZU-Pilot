            ...
            <div class="flex flex-col h-full">
                <div class="flex-grow space-y-4">
                    ...
                    <li>
                     ...
                     <li>
                        <button data-view="students" class="nav-item p-3 rounded-lg flex items-center justify-center text-green-200 hover:bg-green-700 hover:text-white" title="Správa studentů">
                ...
                <div>
                    <li>
                         <button id="logout-btn-nav" class="nav-item p-3 rounded-lg flex items-center justify-center text-green-200 hover:bg-red-700 hover:text-white" title="Odhlásit se">
...
async function showProfessorContent(view, lesson = null) {
    ...
    } else if (view === 'media' || view === 'students' || view === 'interactions') {
...
function renderScheduledEvent(event) {
    ...
    return `
    <div class="lesson-bubble p-3 rounded-lg shadow-sm flex items-center justify-between border bg-green-50 text-green-800 border-green-200" data-event-id="${event.id}">
    ...
    container.innerHTML = `
        ...
        <div class="flex-grow overflow-y-auto p-4 md:p-6">
            <div class="bg-white p-6 rounded-2xl shadow-lg">
                ...
                <div id="course-media-upload-area" class="border-2 border-dashed border-slate-300 rounded-lg p-10 text-center text-slate-500 cursor-pointer hover:bg-green-50 hover:border-green-400"><p class="font-semibold">Přetáhněte soubory sem nebo klikněte pro výběr</p></div>
...
async function renderTelegramInteractionView(container) {
    ...
    try {
        const studentsCollection = collection(db, 'students');
        const querySnapshot = await getDocs(query(studentsCollection, where("telegramChatId", "!=", null)));
        const connectedStudents = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        ...
        const studentCardsHtml = connectedStudents.map(student => `
            <div class="bg-white rounded-2xl shadow-lg p-6 flex flex-col" data-student-id="${student.id}">
                <div class="flex items-center mb-4">
                    <p class="font-semibold text-slate-700 truncate">${student.name || student.email}</p>
                </div>
