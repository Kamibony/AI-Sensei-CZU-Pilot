...
function renderLessonLibrary(container, showContentCallback) {
    ...
    container.querySelectorAll('.lesson-bubble-in-library').forEach(el => {
        el.addEventListener('click', (e) => {
            ...
            showContentCallback('editor', selectedLesson);
        });
    });
    ...
    container.querySelector('#add-new-lesson-btn').addEventListener('click', () => showContentCallback('editor', null));
...
async function showProfessorContent(view, data = null) {
    ...
    switch (view) {
        ...
        case 'interactions':
            sidebar.style.display = 'none';
            conversationsUnsubscribe = renderStudentInteractions(mainArea, db, functions);
            break;
        case 'analytics':
             sidebar.style.display = 'none';
             mainArea.innerHTML = `<p class="p-8 text-center text-slate-500">Sekce Analýza se připravuje.</p>`;
            break;
        default: // 'timeline'
            await fetchLessons();
